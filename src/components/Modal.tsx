...
function Modal(props: ModalProps) {
            ...
            case "General":
                return (
                    ...
                    >
                        ...
                        <select class="bg-white text-black text-xl rounded-lg p-2"
                        ...
                        >
                            <For each={themes}>
                                {theme => (
                                    <option
                                        selected={theme.name === props.modalState.theme}
                                    >{theme.name}</option>
                                )}
                            </For>
                        </select>
                        ...
                        >
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" class="sr-only peer"
                                       onClick={() => props.setModalState({cycle: !props.modalState.cycle})}
                            ...
                            <div class="flex flex-col space-x-1">
                                <span>Use the Pomodoro sequence: Pomodoro → short break, repeat 4x, then one long break</span>
                                <span class="text-gray-500">Number of Pomodoros complete is indicated with dots under 'Pomodoro'</span>
                            </div>
                        </div>
                    </div>
                );
                    ...
                    <div class="flex flex-col space-y-4 w-full">
                        ...
                        >
                            <For each={audios}>
                                {audio_item => (
                                    <option
                                        selected={audio_item.name === props.modalState.audio}
                                    >{audio_item.name}</option>
                                )}
                            </For>
                        </select>
                    </div>
    ...
    return (
        ...
        >
            <div
                class="bg-black text-white p-5 flex flex-col justify-center items-center rounded-xl w-[500px] h-[480px] space-y-4"
                ...
                <header class="w-full flex justify-between space-x-10 p-2 h-full">
                    <div class="flex flex-col space-y-4">
                        <Button selectType={setSelectTab} type={selectTab} name="General" small={true}/>
                        <Button selectType={setSelectTab} type={selectTab} name="Timers" small={true}/>
                        <Button selectType={setSelectTab} type={selectTab} name="Sounds" small={true}/>
                    </div>
                    {changeTab(selectTab())}
                </header>
                <footer
                    class="w-full flex justify-end mt-4 space-x-4"
                >
                    <button
